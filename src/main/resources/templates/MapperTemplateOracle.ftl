...
<mapper namespace="${bussPackage}.${entityPackage}.dao.${className}Dao">
    ...
    <resultMap id="BaseResultMap" type="${bussPackage}.${entityPackage}.entity.$!{className}">
        ...
        <id column=$po.oracleColumnName property="$po.dataName"/>
        #end
        #foreach($po in ${notPkColumnDatas})
        <result column=$po.oracleColumnName property="$po.dataName"/>
        #end
    </resultMap>
    ...
    <sql id="Base_Where_Clause">
        ...
        <trim suffixOverrides=",">
            ...
            <if test="$!{po.dataName} != null">
                and $!{po.oracleColumnName} ${$!{po.dataName}Operator} #{$!{po.dataName}}
            </if>
            #if(${po.shortDataType} == 'Date')
            <if test="$!{po.dataName}1 != null">
                and $!{po.oracleColumnName} &gt;= #{$!{po.dataName}1}
            </if>
            <if test="$!{po.dataName}2 != null">
                and $!{po.oracleColumnName} &lt;= #{$!{po.dataName}2}
            </if>
            #end
            #end
        </trim>
    ...
    <insert id="insertSelective" parameterType="${bussPackage}.${entityPackage}.entity.${className}">
        ...
        <trim prefix="(" suffix=")" suffixOverrides=",">
            #foreach($po in $!{columnDatas})
            <if test="$!{po.dataName} != null">$!{po.oracleColumnName},</if>
            #end
        </trim>
    ...
    <update id="updateByPrimaryKey" parameterType="${bussPackage}.${entityPackage}.entity.${className}">
        update ${tableName}
        #foreach($po in ${notPkColumnDatas})
        #if($velocityCount == 1)set #else #end ${po.oracleColumnName} = #{${po.dataName}}#if($velocityCount
        < ${notPkColumnDatas.size()}),
        #end
        #end

        #foreach($po in ${pkColumnDatas})
        #if($velocityCount == 1)where #else and #end ${po.oracleColumnName} = #{${po.dataName}}
        #end
    </update>
    ...
    <update id="updateByPrimaryKeySelective" parameterType="${bussPackage}.${entityPackage}.entity.${className}">
        ...
        <set>
            #foreach($po in ${notPkColumnDatas})
            <if test="${po.dataName} != null">
            ${po.oracleColumnName} = #{${po.dataName}},
            </if>
            #end
        </set>
        #foreach($po in ${pkColumnDatas})
    ...
    <select id="selectByPrimaryKey" resultMap="BaseResultMap"
        ...
        from ${tableName}
        #if($pkColumnCount == 1)
        where ${pkColumn.oracleColumnName} = #{value}
        #else
        #foreach($po in ${pkColumnDatas})
        #if($velocityCount == 1)where #else and #end ${po.oracleColumnName} = #{${po.dataName}}
        #end
